...
struct VAO *create3DObject(GLenum primitive_mode, int numVertices, const GLfloat *vertex_buffer_data, const GLfloat *color_buffer_data, GLenum fill_mode) {
    ...
    for (int i = 0; i < numVertices; i++) {
        if(i%3 == 0)
        {
            nor = glm::cross(glm::vec3(
                vertex_buffer_data[3*i] - vertex_buffer_data[3*(i+1)],
                vertex_buffer_data[3*i+1] - vertex_buffer_data[3*(i+1)+1],
                vertex_buffer_data[3*i+2] - vertex_buffer_data[3*(i+1)+2]
            ),glm::vec3(

                vertex_buffer_data[3*i] - vertex_buffer_data[3*(i+2)],
                vertex_buffer_data[3*i+1] - vertex_buffer_data[3*(i+2)+1],
                vertex_buffer_data[3*i+2] - vertex_buffer_data[3*(i+2)+2]
            ));
        }
        else if(i%3 == 1)
        {
...
void draw3DObject(struct VAO *vao) {
    ...
    // Enable Vertex Attribute 2 - Normal
